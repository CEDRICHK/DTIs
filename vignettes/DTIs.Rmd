...
```{r usage, eval=FALSE}
DTIs::uniprot_drug_data(drug = c("Aceclofenac", "Alverine"))
```
...
targets <- DTIs::uniprot_drug_data(drug)
...
If you want to persist the result, prefer using an explicit path handled by the
`fs` package so that the workflow stays reproducible and portable:

```{r SaveExample, eval=FALSE}
fs::dir_create("analysis")
fs::path("analysis", "targets.csv") |>
  write.csv(targets, row.names = FALSE)
```

This snippet creates an `analysis/` folder (if missing) and writes the CSV into
it, keeping generated artefacts outside the package root.
...
fs::dir_create("analysis")
readr::write_csv(targets, fs::path("analysis", "batch_targets.csv"))
```
